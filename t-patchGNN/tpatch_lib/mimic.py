...
class MIMIC(object):
    ...
    def __init__(self, root, n_samples = None, device = torch.device("cpu")):
        ...
        if device == torch.device("cpu"):
            ...
            self.data = torch.load(os.path.join(self.processed_folder, 'mimic.pt'), map_location=torch.device('cpu'))
    ...
    def process(self):
        ...
        data_gp = full_data.groupby('HADM_ID') # group by index

        for record_id, data in data_gp:
            # tt = torch.tensor(data['TIME_STAMP'].values).to(self.device).float() / 60.
            tt = torch.tensor(pd.to_timedelta(data['TIME_STAMP']).dt.total_seconds().values).to(self.device).float() / 3600.
